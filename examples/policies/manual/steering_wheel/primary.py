...
class WheelController(Controller):
    """
    A controller class designed to interface with a QCar model using a Logitech steering wheel
    controller. This class is not compatible with other qcar modules, run this in a separate process
    ...
    Attributes:
        ...
        controller (LogitechController): An instance of the LogitechController to interact with the physical
            device.
        control_strategies (List[WheelControllerStrategy]): A list of strategies for controlling various
            flags like safety, reverse, light, and cruise.
    ...
    Methods:
        ...
        execute(queue: Queue): Updates the state of the QCar model based on input from the wheel controller
            and applies control strategies.
    ...
    def execute(self, queue: Queue) -> None:
        if self.controller.logi_update(): # update every frame
            state_engines = self.controller.get_state_engines(self.index) # get input from the controller
